    ...
    static func main() throws {
        ...
        for y in stride(from: 0, to: pixels.pixelsHigh, by: 2) {
            for x in stride(from: 0, to: pixels.pixelsWide, by: 2) {
                ...
                // Test red DOMINANCE, not absolute channel values. Rendering a
                // PDF page through NSImage applies colour management, and on
                // macOS 26 pure sRGB red samples back as roughly (0.94, 0.29,
                // 0.18) in deviceRGB - so the original `green < 0.2` bound
                // failed even though the image rendered correctly. Dominance
                // still separates the red attachment from white paper
                // (r == g == b) and black text (low red).
                if color.redComponent > 0.8
                    && color.redComponent - max(color.greenComponent, color.blueComponent) > 0.4 {
                    foundRedPixel = true
                    break
                }
            }
