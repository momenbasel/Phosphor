...
def test_incremental_backups_require_existing_metadata(root: Path) -> None:
    ...
    assert_contains(manager, "static func deleteIncompleteBackup(", "Recovery flow should expose guarded interrupted-backup cleanup")
    assert_contains(manager, ") async throws {", "Incomplete-backup recovery must await destination identity preflight before Trash")
